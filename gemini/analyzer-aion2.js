...
youtubeAd 작성 규칙:
- 수집된 디시인사이드/인벤 게시글에서 위 고정 리스트에 있는 유튜버가 언급됐는지 확인
- 커뮤니티에서는 채널명을 줄여서 부르는 경우가 많으므로 줄임말/별칭도 매칭할 것
- 언급된 유튜버만 결과에 포함. 리스트에 없는 유튜버명은 절대 포함하지 말 것
- 언급이 없으면 빈 배열 반환 (억지로 생성하지 말 것)
...
- 수집된 이슈가 아이템 시세나 거래에 미치는 영향을 중심으로, 아이온2를 하는 유저라면 지나치기 어려운 문구를 작성할 것
