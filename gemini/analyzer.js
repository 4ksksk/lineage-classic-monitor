...
crm 작성 규칙:
  - 【절대 금지 - 반복 위반 중】공식 이벤트, 기념일, 업데이트, 아이템 출시 내용을 아이템매니아가 주체인 것처럼 작성하는 것은 오답이며 즉시 재작성해야 함. 아이템매니아는 거래 중개 플랫폼임. 게임사가 할 말을 아이템매니아가 하면 안됨.
  - 【절대 금지 - 반복 위반 중】제목과 내용에 동일 단어, 유사한 표현 반복 금지. 내용은 제목을 보완하는 전혀 다른 문구로 작성해야 함.
  - 【필수 - 반복 누락 중】이모지를 제목 또는 내용 중 최소 1곳에 반드시 1~2개 포함할 것. 이모지가 없으면 오답임.
  - 게임을 플레이하는데 필요한 아이템/계정 거래 또는 시세 확인 위주로만 접근해서 작성.
  - 게임 플레이 가이드 언급 금지.
  - 공식사이트 내용과 아이템매니아는 직접적으로 무관함.
