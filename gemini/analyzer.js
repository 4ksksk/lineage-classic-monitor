...
push 작성 규칙:
- 반드시 이번에 수집된 데이터에 실제로 존재하는 이슈·업데이트·아이템명·시세 내용만 사용할 것
- 수집된 데이터에 없는 이벤트, 혜택, 상품 출시, 한정 판매 등을 있는 것처럼 표현하는 것은 절대 금지
- 이 플랫폼은 아이템을 직접 판매·유통하지 않으며 판매자와 구매자를 연결하는 중개 서비스임을 반드시 인지할 것
- 플랫폼이 직접 상품을 출시하거나 이벤트를 주최하거나 해결 방안을 제공하는 주체처럼 작성하지 말 것
- 자동작업장·핵·매크로 등 불법·부정 이슈는 광고 소재로 절대 사용 금지
- 제목은 수집된 실제 이슈·아이템명을 훅으로 활용해 유저의 눈길을 끌 것
- 내용은 제목을 보완하며 "지금 시세 확인", "매물 탐색" 등 클릭해야 하는 이유를 구체적으로 제시할 것
- 말투는 게임 유저에게 말하는 것처럼 가볍고 친근하게 작성할 것
- "즐겨보세요", "살펴보세요", "높이세요" 등 딱딱하고 게임사 공지 같은 표현 금지
- 근거 없는 마케팅 관용 문구 사용 금지
- 이모지는 1~2개, 문장 흐름에 자연스럽게 삽입할 것 (과하면 금지)
