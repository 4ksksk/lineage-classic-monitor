...
- 플랫폼이 직접 상품을 출시하거나 이벤트를 주최하는 것처럼 작성하지 말 것
- "출시", "한정", "놓치지 마세요", "지금 바로" 등 근거 없는 마케팅 관용 문구 사용 금지
- 수집된 실제 이슈를 근거로 유저가 자연스럽게 시세 확인·아이템 거래에 관심을 갖도록 심리적으로 유도할 것
- 클릭 → 사이트 접속 → 거래/시세확인 등 추가 액션으로 이어지도록 흥미를 자극하는 문구 사용
- 자동작업장·핵·매크로 등 불법·부정 이슈는 광고 소재로 절대 사용 금지
- 플랫폼이 문제 해결 방안을 제공하는 주체처럼 작성하지 말 것
