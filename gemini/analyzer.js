...
crm 작성 규칙:
- 아이템매니아는 게임 유저들이 게임 내 아이템 및 계정을 사고팔 수 있는 중개 거래 플랫폼이다. 아이템을 직접 제조·판매·유통하지 않으며, 판매자와 구매자를 연결해주는 역할만 한다. 이 푸시는 아이템매니아가 특정 게임 유저에게 발송하는 광고 알림이다.
- 푸시의 목적은 특정 게임 유저가 지금 아이템매니아에 접속해서 시세를 확인하거나 아이템 거래를 하도록 유도하는 것이다.
- 아이템매니아는 판매자와 구매자를 연결하는 중개 플랫폼임을 반드시 인지할 것
- 마케팅 제안이나 없는 이벤트/혜택을 사실인 것처럼 쓰지 말 것
- 수집된 실제 이슈/업데이트 내용을 기반으로 작성
- 반드시 이번에 수집된 데이터에 실제로 존재하는 이슈·업데이트·아이템명·시세 내용을 기반으로 작성할 것
- 불법·부정 이슈는 광고 소재로 절대 사용 금지
- 수집된 이슈가 아이템 시세나 거래에 미치는 영향을 중심으로, 특정 게임을 하는 유저라면 지나치기 어려운 문구를 작성할 것
- 해당 이슈로 인해 유저가 자연스럽게 시세 확인, 아이템 거래에 관심을 가질 수 있도록 유도
- 클릭 후 사이트 접속, 더 나아가 거래/시세확인 등 추가 액션까지 이어지도록 심리적으로 유도하는 문구로 작성
...
- 이모지는 제목 또는 내용 중 반드시 1개 이상 포함할 것, 단 과하면 금지
- 과장이나 허위 내용 절대 금지
