...
const fs = require('fs');
const { runCrawlAndAnalyze, runAion2CrawlAndAnalyze, runMapleCrawlAndAnalyze, startScheduler } = require('./scheduler/index');
...
app.get('/api/maple-history', (req, res) => {
  if (!fs.existsSync(HISTORY_DIR_MAPLE)) return res.json([]);
  try {
    const files = fs.readdirSync(HISTORY_DIR_MAPLE)
      .filter(f => /^\d{4}-\d{2}-\d{2}-\d{2}\.json$/.test(f))
      .sort()
      .reverse();
    res.json(files);
...
app.get('/api/maple-history/:filename', (req, res) => {
  const filename = req.params.filename;
  if (!/^\d{4}-\d{2}-\d{2}-\d{2}\.json$/.test(filename))
    return res.status(400).json({ error: '잘못된 파일명' });
  const filepath = path.join(HISTORY_DIR_MAPLE, filename);
  if (!fs.existsSync(filepath)) return res.status(404).json({ error: '파일 없음' });
  try {
    res.json(JSON.parse(fs.readFileSync(filepath, 'utf-8')));
  } catch {
...
app.get('/api/maple-results', (req, res) => {
  if (!fs.existsSync(DATA_PATH_MAPLE)) return res.json(EMPTY);
  try {
    res.json(JSON.parse(fs.readFileSync(DATA_PATH_MAPLE, 'utf-8')));
  } catch {
    res.status(500).json({ error: '데이터 파일을 읽을 수 없습니다.' });
  }
});
