...
const GALLERY_URL  = 'https://gall.dcinside.com/board/lists/?id=maplestory_new';
...
async function crawlMapleDCInside() {
  const posts = [];
  try {
    ...
    console.log(`[메이플스토리 DC인사이드] ${posts.length}개 수집 (본문 포함)`);
  } catch (err) {
    console.error('[메이플스토리 DC인사이드 크롤링 오류]', err.message);
  }
  return posts;
}

module.exports = { crawlMapleDCInside };
