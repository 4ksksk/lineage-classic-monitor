...
const BOARD_URL = 'https://www.inven.co.kr/board/maple/5974';
...
async function crawlMapleInven() {
  const posts = [];
  try {
    ...
    console.log(`[메이플스토리 인벤] ${posts.length}개 수집 (본문 포함)`);
  } catch (err) {
    console.error('[메이플스토리 인벤 크롤링 오류]', err.message);
  }
  return posts;
}

module.exports = { crawlMapleInven };
